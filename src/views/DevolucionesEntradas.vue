...
<q-btn @click="showBtn = true; card = true" icon="add">Agregar Devoluciones Entradas</q-btn>


<q-table title="DEVOLUCIONES ENTRADAS" :rows="rows" :columns="columns" row-key="name" class="tabla">
...
<q-btn @click="Showmodal = false; agregarArrayArticulos()" >Listo</q-btn>
 ...
 <q-card-actions align="right">
  <q-btn @click="agregarDevolucionEntrada(); showBtn = false" v-show="showBtn == true" v-close-popup flat color="primary" label="Agregar" />
...
const dataDevolucionesEntradas = async () => {
 try {
   ...
   console.log("no llegaron las devoluciones entradas" + error.message);
 }
};


const agregarDevolucionEntrada = async () => {
 try {
